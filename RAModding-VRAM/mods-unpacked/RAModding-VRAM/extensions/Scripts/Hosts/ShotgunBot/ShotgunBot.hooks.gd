...
func handle_skin(chain: ModLoaderHookChain):
	...
	if not base_obj.upgrades.is_empty(): # Handle Elites
		...
		base_obj.enemy_fx.get_node("CASHParticles").emitting = Engine.get_singleton("VRAM_SkinsManager").get_player_skin(Enemy.EnemyType.SHOTGUN)["name"] == "CASH";
		return
...
func set_skin_from_skin_manager(base_obj: ShotgunBot, skin_id: String):
	var skin = Engine.get_singleton("VRAM_SkinsManager").get_skin_or_default(Enemy.EnemyType.SHOTGUN, skin_id)
