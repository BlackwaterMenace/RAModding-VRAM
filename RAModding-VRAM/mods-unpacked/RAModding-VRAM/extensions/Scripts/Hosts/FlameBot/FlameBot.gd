...
func handle_skin():
	...
	if not upgrades.is_empty(): # Handle Elites
		for curr_upgrade in upgrades:
			var skin_id = VRAM_SkinsManager.upgrade_skins[Enemy.EnemyType.FLAME].get(curr_upgrade)
			if skin_id != null:
				set_skin_from_skin_manager(skin_id)
				return
	elif is_player:
		set_skin_from_skin_manager(VRAM_SkinsManager.get_player_skin_id(Enemy.EnemyType.FLAME))
		return
	set_skin_from_skin_manager("0") # Handle normal enemies

func set_skin_from_skin_manager(skin_id):
	var skin = VRAM_SkinsManager.get_skin_or_default(Enemy.EnemyType.FLAME, skin_id)
