...
func handle_skin():
	...
	if not upgrades.is_empty():
		for curr_upgrade in upgrades:
			var skin_id = Engine.get_singleton("VRAM_SkinsManager").upgrade_skins[Enemy.EnemyType.SABER].get(curr_upgrade)
			if skin_id != null:
...
func set_skin_from_skin_manager(skin_id):
	var skin = Engine.get_singleton("VRAM_SkinsManager").get_skin_or_default(Enemy.EnemyType.SABER, skin_id)
