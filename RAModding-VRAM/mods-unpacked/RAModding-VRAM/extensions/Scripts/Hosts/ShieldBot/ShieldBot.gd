...
func handle_skin():
	if is_player:
		if GameManager.player_collider_skin_path != "":
			set_skin_from_skin_manager(VRAM_SkinsManager.get_player_skin_id(Enemy.EnemyType.SHIELD))
			return
	set_skin_from_skin_manager("0")

func set_skin_from_skin_manager(skin_id):
	var skin = VRAM_SkinsManager.get_skin_or_default(Enemy.EnemyType.SHIELD, skin_id)
